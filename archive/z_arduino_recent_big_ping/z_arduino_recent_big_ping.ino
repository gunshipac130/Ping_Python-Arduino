...
// WiFi Ping
const char* ssid = "10-Dah_TO_2.4G";
const char* password = "1234asdf";
const IPAddress remote_ip(119, 252, 37, 13);  //datatype: IPAddress; variable name: remote_ip
...
void Job_Setup_Ping() {
  WiFi.begin(ssid, password);  //WiFi "SSID name","WiFi password"
  Serial.print(F("\nConnecting"));
  while (WiFi.status() != WL_CONNECTED) {
    delay(1000);
    Serial.print(".");
    Work_OLED_Ping_Setup_Info();
  }
  big_ping_timestamp = 0;
  Serial.print(F("\n\nConnected, Local IP: "));
...
void Job_Loop_Ping() {
  ...
  if (Ping.ping(remote_ip, 1) > 0) {  // Ping.ping have internal serial debug from esp8266.h or something
    // ping 1 time(s) to remote_ip
    // Serial.printf(" response time : %d/%.2f/%d ms\n", Ping.minTime(), Ping.averageTime(), Ping.maxTime());
    if (Ping.maxTime() <= 999) {
      Work_Ping_push_array(ping_array, Ping.maxTime(), ping_array_size);
    } else {
  ...
  Work_Ping_big_check(ping_array_big, Ping.maxTime(), ping_array_big_size);
...
void Work_Ping_big_check(int arr[], int big_ping, int array_size) {  // array, new ping 140+ ms, array_size
  if (Ping.maxTime() >= 140) {                                       // if ping 140+ms, push array and fill new ping
...
/*
Changelog:
202312112300 add 1 extra pixel for ping y axis if ping 121+ms for more every 10ms
202312120443 add recent high ping (140+) at bottom OLED
